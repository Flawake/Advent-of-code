...
fn calculate(map: Vec<Vec<u8>>, mut trail_heads: Vec<TrailHead>) -> i32 {
    trail_heads.iter_mut().for_each(|head| {
        println!("iter");
        head.score = find_nine_count(&map, (head.x as usize, head.y as usize), &mut vec![]);
    });
    trail_heads.iter().map(|head|head.score).sum()
}

fn find_nine_count(map: &Vec<Vec<u8>>, start_pos: (usize, usize), found_nine_positions: &mut Vec<(usize, usize)>) -> i32 {
    ...
    if current_value == 9 && !found_nine_positions.contains(&(row, col)) {
        found_nine_positions.push((row, col));
        return 1;
    }
    ...
    if col < map[0].len() - 1 && map[row][col + 1] == current_value + 1 {
        count += find_nine_count(map, (row, col + 1), found_nine_positions);
    }
    if col > 0 && map[row][col - 1] == current_value + 1 {
        count += find_nine_count(map, (row, col - 1), found_nine_positions);
    }
    if row < map.len() - 1 && map[row + 1][col] == current_value + 1 {
        count += find_nine_count(map, (row + 1, col), found_nine_positions);
    }
