...
fn calculate(input: &Vec<(u16, u16)>) -> (u16, u16) {
    let width = 71;
    let heigth = 71;
    let mut map = vec![vec![u8::MIN; width]; heigth];

    for i in input {
        map[i.1 as usize][i.0 as usize] = 1;

        let start_pos = (0, 0);
        let end_pos = (width as u8 - 1, heigth as u8 - 1);

        if find_shortest_path(&map, start_pos, end_pos) == None {
            return *i;
        }
    }
    (0, 0)
...
fn find_shortest_path(map: &Vec<Vec<u8>>, start_pos: (u8, u8), end_pos: (u8, u8)) -> Option<u32> {
